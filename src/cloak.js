...
// 15 February 2016
//
// Run: live-server --ignore="C:\Users\frank\Desktop\DEV\cloak-of-darkness\src"
...
const CloakOfDarkness = (function() {
	...
	_this.story = [
		...
		{ id: _this.ids.win, text: "You read the message on the ground and it says 'You Won!'" },
	...
	_this.actions = [
		...
		{
			// Actions that can happen in any room but are specific to the actual game content (not generic system actions)
			commands: [
				{
					synonyms: ["drop cloak"], 
					perform: function(player) {           
						if(_.indexOf(player.items, "cloak of darkness") > -1) {  
							...
							if(player.roomState[player.room.id] === undefined) {
								let _state = {}
							} 
...
class IFGameEngine extends React.Component {
	constructor() {
		...
		this.state = {
			...
			player: {
				room: { name: "" },
				roomState: {}, // right now will be used for objects we drop                   
				score: 0,
				moves: 0,
				items: [],
				flags: []         
...
$(document).ready(function() {
  ReactDOM.render(
		<IFGameEngine title="Cloak of Darkness" 
